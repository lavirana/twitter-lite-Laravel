...
<?php } else {  ?>
                         <img style="width:50px;" src="{{$tweet->user->avatar}}" class="rounded-full mr-2">
                          <?php } ?> 
                    </a>
            </div>
            <div>
                ...
                <p class="text-sm mb-3">{{$tweet->body}}</p>
                  <div class="flex">
                      ...
                      @csrf
              <div class="flex item-center mr-4 {{ $tweet->isLikedBy(current_user()) ? 'text-blue-500' : 'text-gray-500'}}">
                  ...
                  {{$tweet->isLikedBy(current_user()) ?: 0 }}
                  </button>
                  </div>
                      ...
                      @csrf
                              @method('DELETE')
                            <div class="flex item-center {{ $tweet->isDislikedBy(current_user()) ? 'text-blue-500' : 'text-gray-500'}}">
                 <svg viewBox="0 0 20 20" class="mr-1 w-3">
						<g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
							<g class="fill-current">
                  ...
                  <button type="submit" class="text-xs text-gray-500">
                                {{ $tweet->isDislikedBy(current_user())  ?: 0 }}
                                </button>
                  </div>
                      </form>
              </div>
